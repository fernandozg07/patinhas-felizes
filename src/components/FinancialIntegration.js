  ...
  return (
    <div className="min-h-screen p-6" style={{
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="rounded-2xl p-8 mb-8" style={{
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-600">Gestão Financeira em Tempo Real - Patinhas Felizes</p>
            </div>
            <div className="flex gap-3">
              <button className="px-6 py-3 rounded-xl text-white font-semibold transition-all duration-300" style={{
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 rounded-2xl p-8" style={{
            ...
            <div className="space-y-4">
              {revenueStreams.map(stream => (
                <div key={stream.id} className="p-4 rounded-xl transition-all duration-300 hover:scale-105" style={{
                  background: 'linear-gradient(135deg, #f0fdf4, #dcfce7)',
                  border: '1px solid #bbf7d0'
                }}>
                  <div className="flex-1">
                    <div className="flex justify-between items-center mb-2">
                      <h3 className="font-bold text-gray-800">{stream.source}</h3>
                    ...
                    <div className="flex justify-between items-center mb-3">
                      <span className="text-2xl font-bold" style={{ color: '#059669' }}>{formatCurrency(stream.amount)}</span>
                      <span className="text-sm font-medium text-gray-600">{stream.percentage}% do total</span>
                    </div>
            ...
            <div className="mt-6 p-6 rounded-2xl" style={{
              ...
              <div className="flex justify-between items-center">
                <div>
                  <h3 className="font-bold text-gray-800 text-lg">Total de Receitas</h3>
                  <p className="text-3xl font-bold" style={{ color: '#059669' }}>
                    {formatCurrency(revenueStreams.reduce((sum, stream) => sum + stream.amount, 0))}
                  </p>
                  <p className="text-sm text-gray-600 mt-1">Meta mensal atingida: +15%</p>
                </div>
          ...
          <div className="rounded-2xl p-6" style={{
            ...
            <div className="mt-6 space-y-3">
              <h3 className="font-bold text-gray-800">⚡ Métricas Rápidas</h3>
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-blue-50 p-3 rounded-lg text-center">
                  <p className="text-lg font-bold text-blue-600">2.4</p>
        ...
        <div className="mt-6 rounded-2xl p-8" style={{
          background: '#ffffff',
          boxShadow: '0 4px 6px rgba(0, 0, 0, 0.05)',
          border: '1px solid #e5e7eb'
        }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {expenses.map(expense => {
              ...
              return (
                <div key={expense.id} className="p-6 rounded-xl transition-all duration-300 hover:scale-105" style={{
                  ...
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <span className="font-bold text-lg" style={{ color: colors[expense.color].text }}>{expense.percentage}%</span>
                  </div>
                  <p className="text-2xl font-bold mb-3" style={{ color: colors[expense.color].text }}>{formatCurrency(expense.amount)}</p>
          ...
          <div className="mt-8 p-6 rounded-2xl" style={{
            ...
            <div className="flex justify-between items-center">
              <div>
                <h3 className="font-bold text-gray-800 text-lg">Total de Custos Mensais</h3>
                <p className="text-3xl font-bold" style={{ color: '#dc2626' }}>
                  {formatCurrency(expenses.reduce((sum, expense) => sum + expense.amount, 0))}
                </p>
                <p className="text-sm text-gray-600 mt-1">88% Pessoal | 9% Estrutura | 3% Insumos</p>
              </div>
              <div className="text-5xl">📊</div>
            </div>
          </div>
        ...
        <div className="mt-6 bg-white rounded-xl shadow-lg p-6">
          <h2 className="text-xl font-bold text-gray-800 mb-4">🔮 Projeção Financeira - Próximos 6 Meses</h2>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-100">
                  <th className="p-3 text-left">Mês</th>
                  <th className="p-3 text-right text-green-600">Receita Projetada</th>
                  <th className="p-3 text-right text-red-600">Custos</th>
                  <th className="p-3 text-right text-blue-600">Lucro Líquido</th>
              ...
              <tbody>
                {[
                  ...
                  <tr key={index} className="border-b hover:bg-gray-50">
                    ...
                    <td className="p-3 text-right text-green-600 font-bold">{formatCurrency(row.revenue)}</td>
                    <td className="p-3 text-right text-red-600">{formatCurrency(row.cost)}</td>
                    <td className="p-3 text-right text-blue-600 font-bold">{formatCurrency(row.profit)}</td>
